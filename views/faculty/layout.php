...
<head>
    ...
    <title><?= ucfirst($currentPage) ?> - Faculty Panel | <?= APP_NAME ?></title>
...
<body class="bg-gray-100 font-sans antialiased">
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 flex flex-col">
            <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-700/50">
                ...
                <div>
                    ...
                    <div class="text-slate-400 text-xs">Faculty Panel</div>
                </div>
            </div>

            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                <a href="<?= APP_URL ?>/faculty/dashboard" class="sidebar-link <?= $currentPage === 'dashboard' ? 'active' : '' ?>">
                    <i class="fas fa-th-large w-5 text-center"></i>
                    <span class="sidebar-text ml-3">Dashboard</span>
                </a>

                <div class="pt-4 pb-1">
                    <span class="sidebar-text text-xs font-semibold text-slate-500 uppercase tracking-wider px-3">Teaching</span>
                </div>

                <a href="<?= APP_URL ?>/faculty/classes" class="sidebar-link <?= $currentPage === 'classes' ? 'active' : '' ?>">
                ...
                <a href="<?= APP_URL ?>/faculty/exams" class="sidebar-link <?= $currentPage === 'exams' ? 'active' : '' ?>">
                    <i class="fas fa-file-alt w-5 text-center"></i>
                    <span class="sidebar-text ml-3">Exams</span>
                </a>

                <a href="<?= APP_URL ?>/faculty/scores" class="sidebar-link <?= $currentPage === 'scores' ? 'active' : '' ?>">
                    <i class="fas fa-clipboard-check w-5 text-center"></i>
                    <span class="sidebar-text ml-3">Manage Scores</span>
                </a>
                ...
                <a href="<?= APP_URL ?>/faculty/materials" class="sidebar-link <?= $currentPage === 'materials' ? 'active' : '' ?>">
                    <i class="fas fa-folder-open w-5 text-center"></i>
                    <span class="sidebar-text ml-3">Study Materials</span>
                </a>

                <a href="<?= APP_URL ?>/faculty/schedules" class="sidebar-link <?= $currentPage === 'schedules' ? 'active' : '' ?>">
                    <i class="fas fa-calendar-alt w-5 text-center"></i>
                    <span class="sidebar-text ml-3">My Schedule</span>
                </a>

                <a href="<?= APP_URL ?>/faculty/assignments" class="sidebar-link <?= $currentPage === 'assignments' ? 'active' : '' ?>">
                    <i class="fas fa-tasks w-5 text-center"></i>
                    <span class="sidebar-text ml-3">Assignments</span>
                </a>

                <a href="<?= APP_URL ?>/faculty/announcements" class="sidebar-link <?= $currentPage === 'announcements' ? 'active' : '' ?>">
                ...
                <a href="<?= APP_URL ?>/faculty/profile" class="sidebar-link <?= $currentPage === 'profile' ? 'active' : '' ?>">
            ...
            <div class="border-t border-slate-700/50 p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-slate-400 text-xs">Lecturer</div>
                    </div>
        ...
        <div class="flex-1 lg:ml-64 flex flex-col min-h-screen min-w-0">
            ...
            <main class="flex-1 p-4 lg:p-6 page-content overflow-auto min-w-0">
                <?php require VIEWS_PATH . '/faculty/' . $currentPage . '.php'; ?>
            </main>
        </div>
    </div>
    ...
    <script>
        ...
        async function handleLogout() {
            ...
            if (yes) {
                const data = await API.post('/api/auth/logout', { role: 'lecturer' });
                if (data && data.success) {
                    Toast.success('Logged out');
                    setTimeout(() => window.location.href = APP_URL + '/login/faculty', 1000);
                }
            }
        }
    </script>
