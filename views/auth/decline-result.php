...
<head>
    ...
    <title>Application Declined - AGIT Academy</title>
...
<body class="min-h-screen bg-gray-100 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        <?php if (($declineResult['success'] ?? false)): ?>
            <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-slate-100 flex items-center justify-center">
                <i class="fas fa-check text-2xl text-slate-600"></i>
            </div>
            <h1 class="text-xl font-bold text-gray-900 mb-2">Application Declined</h1>
            <p class="text-gray-600 text-sm mb-6"><?= htmlspecialchars($declineResult['message']) ?></p>
        <?php else: ?>
            ...
            <p class="text-gray-600 text-sm mb-6"><?= htmlspecialchars($declineResult['message'] ?? 'Something went wrong.') ?></p>
        <?php endif; ?>
